...
export default async function getImagesFrom(
  ...
  parentFolder?: string,
  limit: number = 30,
  nextPageTokenProp: string | undefined = undefined,
) {
  try {
    ...
    const findTheFolder = await findFolderAndCheckWeb({
      targetName: targetFolder.replaceAll('-', ' '),
      parentFolder: parentFolder?.replaceAll('-', ' ') || undefined,
    });
